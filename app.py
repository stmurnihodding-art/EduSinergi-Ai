...
# 2. Styling UI Mobile-First & Responsif (PC, Tablet, Ponsel)
st.markdown("""
<style>
...
/* Judul Adaptif Mengikuti Lebar Layar */
.hero-title {
...
/* Tombol Nyaman Disentuh di Layar Sentuh */
...
/* Kotak Penampung Pratinjau Kosong */
.preview-empty-box {
...
/* Penyesuaian Ruang Layar Ponsel (< 768px) */
...
# 5. Form Input & Parameter Studio (Responsif)
...
with col_in1:
    with st.container(border=True):
        ...
        uploaded_file = st.file_uploader(
            ...
            help="Unggah foto referensi atau naskah acuan."
        )
...
with col_in2:
    with st.container(border=True):
        ...
        st.markdown("""
        * **TikTok / Reels (9:16):** Prioritaskan *Hook* 3 detik pertama dengan visual dinamis dan teks tegas di layar.
        * **Instagram Feed (1:1):** Pastikan kontras warna seimbang dan tidak terlalu padat tulisan.
        * **AI Video Generator:** Salin prompt bahasa Inggris dari hasil naskah ke generator video (seperti Kling AI, Luma Dream Machine, atau Runway).
        """)
    ...
    if not api_key:
        ...
        try:
            ...
            sys_inst = (
                ...
                "Susun hook 3 detik yang memikat, rincian adegan per detik, naskah narasi/voiceover, rekomendasi audio, "
                "tagar relevan, serta prompt teks-ke-video dalam Bahasa Inggris."
            )

            prompt_konten = (
                f"Platform Target: {platform}\n"
                f"Format Rasio: {aspek_rasio}\n"
                f"Gaya Visual: {gaya_visual}\n"
                f"Detail Konten: {topik_konten}\n\n"
                "Susun rencana visual dan naskah produksi secara rapi dan siap pakai:"
            )
            ...
            with st.spinner("⚡ Merender visual desain dan menyusun panduan konten..."):
                # A. Render Gambar Desain (Imagen 3)
                try:
                    img_prompt = f"Professional social media photography and graphic for {platform}, style: {gaya_visual}, theme: {topik_konten}, clean layout, crisp details, 8k resolution"
...
# 7. Tampilan Pratinjau & Pusat Unduhan
st.divider()
if st.session_state.res_img_bytes or st.session_state.res_text_content:
    ...
    if st.session_state.uploaded_preview:
        with st.expander("📷 Lihat Berkas / Foto Acuan yang Anda Unggah", expanded=False):
            st.image(st.session_state.uploaded_preview, width=280)
    ...
    with tab_view1:
        if st.session_state.res_img_bytes:
            col_v1, col_v2 = st.columns([1.8, 1.2], gap="medium")
            with col_v1:
                st.image(st.session_state.res_img_bytes, caption=f"Desain Siap Pakai ({aspek_rasio})", use_container_width=True)
            with col_v2:
                st.markdown("#### 💾 Simpan Gambar")
                st.write("Visual resolusi tinggi siap diunggah ke media sosial Anda:")
                st.download_button(
            ...
            st.info("Visual gambar tidak diminta atau dalam proses.")
    ...
    with tab_view3:
        ...
        edited_naskah = st.text_area("Editor Teks:", value=st.session_state.res_text_content, height=320)
    ...
    <div class="preview-empty-box">
        <h4>⚡ Area Pratinjau Hasil Kreasi</h4>
        <p>Gambar visual resolusi tinggi, storyboard video TikTok/Reels, serta berkas unduhan Word/PDF akan langsung muncul di sini setelah Anda menekan tombol di atas.</p>
    </div>
    """, unsafe_allow_html=True)
